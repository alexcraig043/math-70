        ...
        if (job == 2) # 3D scatter plot
            {
                ...
                X <- X[, -out]
                nmo <- nm[-out]
                op <- persp(x = range(X[, 1]), y = range(X[, 2]), z = matrix(ncol = 2, nrow = 2), zlim = range(X[, 3]), xlab = nmo[1], ylab = nmo[2], zlab = nmo[3], theta = 35, phi = 40, r = 1000, main = "Fisher iris data", ticktype = "detailed")
        ...
        if (job == 3) # bird view
            {
                for (theta in 1:360)
                {
                    ...
                    fn <- paste("c:\\M7021\\iris360\\ir_", ic, ".png", sep = "")
                    png(fn, width = 1000, height = 1000)
                    par(mfrow = c(1, 1), mar = c(3, 1, 3, 1), cex.lab = 1.4, cex.main = 1.5)
                    X <- as.matrix(iris[, 1:4])
                    X <- X[, -out]
                    nmo <- nm[-out]
                    op <- persp(x = range(X[, 1]), y = range(X[, 2]), z = matrix(ncol = 2, nrow = 2), zlim = range(X[, 3]), xlab = nmo[1], ylab = nmo[2], zlab = nmo[3], theta = theta, phi = 40, r = 1000, main = "Fisher iris data", ticktype = "detailed")
                    p3 <- trans3d(x = X[, 1], y = X[, 2], z = X[, 3], pmat = op)
                    points(p3, pch = 16, cex = 2, col = allcl[cl])
                    n <- length(X[, 1])
                    p2 <- trans3d(x = X[, 1], y = X[, 2], z = rep(min(X[, 3]), n), pmat = op)
                    segments(p3$x, p3$y, p2$x, p2$y)
                    points(p2, pch = 4, cex = 1.5, col = allcl[cl])
                    legend("topright", uflnames, pch = 16, col = allcl, cex = 2.5)
                    dev.off()
                }

                system("magick c:\\M7021\\iris360\\*.png c:\\M7021\\iris.gif")
            }
        print(Sys.time() - t0)
    }
